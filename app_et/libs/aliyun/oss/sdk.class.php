...
define('OSS_VERSION','1.0.3');
define('OSS_BUILD','201206121147');
...
class ALIOSS{
	 ...
	 * OSS服务地址
	 */
	const DEFAULT_OSS_HOST = 'oss-cn-hangzhou.aliyuncs.com';
	...
	public function auth($options){
		...
		foreach ( $headers as $header_key => $header_value ) {
			...
			if (
				...
				(isset($options['self::OSS_EXPIRES']) && (integer) $options['self::OSS_EXPIRES'] > 0)
			){
		...
		if (isset($options[self::OSS_EXPIRES]) && (integer) $options[self::OSS_EXPIRES] > 0){
			return $this->request_url . $conjunction . self::OSS_URL_ACCESS_KEY_ID.'=' . $this->access_id . '&'.self::OSS_URL_EXPIRES.'=' . $options[self::OSS_EXPIRES] . '&'.self::OSS_URL_SIGNATURE.'=' . rawurlencode($signature);
		}elseif (isset($options[self::OSS_EXPIRES])){
	...
	public function get_sign_url($bucket,$object,$timeout = 60,$options = NULL){
		...
		$options[self::OSS_EXPIRES] = $timeout;
