...
//require("images_resize.php");
//
// wx1d665b1493725ca8
// 11eec19d40b46b7f537f5902613b7096
// $YF = new YF_wechat();
...
class YF_wechat
{
    ...
    private function checkSignature()
    {
        ...
        $token = $_SESSION['ApiWeChat']["TOKEN"];
    ...
    {
        $access_token = $this->get_access_token($_SESSION['ApiWeChat']["APPID"],$_SESSION['ApiWeChat']["SECRET"]);
    ...
    public function GetUserInfo($openid)
    {
        ...
        return json_decode($this->curlPostGet($url, 2),TRUE);
    }
    ...
    */
    ...
    public function UploadFile($fileurl, $type)
    {
        ...
        $BASE_PATH =str_replace('class/','',$BASE_PATH);

        $access_token = $this->get_access_token($_SESSION['ApiWeChat']["APPID"],$_SESSION['ApiWeChat']["SECRET"]);
        $json=json_decode($this->curlPostGet($access_token,2));
        $realfile = urldecode($BASE_PATH.$fileurl);
        if($type=='thumb'){
            ...
            $newSrc= $pathinfo[dirname]."/".$fileName."_s.jpg";
            $newimage->resize($realfile,$newSrc,400,300);
    ...
    //curl get post
    function curlPostGet($url, $type = 1, $data = NULL,$xml=null){
        if($type == 1)
        {
            $method = "POST";
        }
...
//$YF = new YF_wechat();
//高级群发接口
//$json=json_decode($YF->UploadFile("/demo1.jpg","thumb"));//缩略图
/*
...
$obj = json_decode($YF->UploadImgTEXT($msg_arr));
var_dump($YF->GroupSend(0, $obj->media_id));
*/
//生成带参数的二维码
//$json=json_decode($YF->MakeQRcode(10, 1800));
//header("Location: ".$YF->GetQRcode($json->ticket));

//$result = $YF->SendCustomMsg("openid","text","测试消息123".$i);
//var_dump($result);

//上传多媒体文件
//var_dump($YF->UploadFile("/demo1.jpg","thumb"));

//下载多媒体文件
//$media = $YF->DownloadFile("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
//$return = $YF->SaveMedia($media,"dream.jpg");
//print_r($return);


//获取用户信息
//$return = $YF->GetUserInfo("openid");
//print_r($return);

//创建分组
//print_r(json_decode($YF->CreateGroup("分组")));

//查询所有分组
//print_r($YF->SearchGroup());

//查询用户所在分组
//echo $YF->SearchUserGroup("openid");

//修改分组名
//print_r(json_decode($YF->UpdateGroupName(100, "分组")));

//移动用户分组
//print_r(json_decode($YF->MoveToGroup("openid", 0)));

//获取关注者列表
//print_r($YF->GetFollowList("openid"));
...
if ($code)
{
	$arr = $YF->get_web_access_token(APPID, SECRET, $code);
	print_r($arr);
}
*/
...
$arr = $YF->refresh_access_token(APPID, $refresh_token);
print_r($arr);
*/

//拉取用户信息(需scope为 snsapi_userinfo)
/*
$userarr = $YF->WebGetUserInfo($arr["access_token"],$arr["openid"]);
echo "<br><br><br><br>";
print_r($userarr);
*/
...
{
	$msg_arr["msg"] = "谢谢使用";
	$YF->SendMsg("text", $msg_arr);
}
*/
//发送图片消息
/*
$json=json_decode($YF->UploadFile("/demo.jpg","image"));
$msg_arr["mediaid"] = $json->media_id;
$YF->SendMsg("image", $msg_arr);
*/
//发送语音消息
/*
$json=json_decode($YF->UploadFile("/voice.amr","voice"));
$msg_arr["mediaid"] = $json->media_id;
$YF->SendMsg("voice", $msg_arr);
*/
...
$json=json_decode($YF->UploadFile("/video.mp4","video"));
$msg_arr["mediaid"] = $json->media_id;
$YF->SendMsg("video", $msg_arr);
*/
...
//{
//$msg_arr["msg"] = "测试消息";
//$YF->SendCustomMsg("openid","text",$msg_arr);
//sleep(1);
//}
//发送客服消息 - 图片消息
/*
$json=json_decode($YF->UploadFile("/demo1.jpg","image"));
$msg_arr["mediaid"] = $json->media_id;
$YF->SendCustomMsg("openid","image",$msg_arr);
*/
//发送客服消息 - 语音消息
/*
$json=json_decode($YF->UploadFile("/voice.amr","voice"));
$msg_arr["mediaid"] = $json->media_id;
$YF->SendCustomMsg("openid","voice",$msg_arr);
*/
//发送客服消息 - 视频消息
/*
$msg_arr["title"] = '测试视频';
...
$json=json_decode($YF->UploadFile("/video.mp4","video"));
$msg_arr["mediaid"] = $json->media_id;
$YF->SendCustomMsg("openid","video",$msg_arr);
*/
//发送客服消息 - 音乐消息
/*
$msg_arr["title"] = 'title';
...
$msg_arr["musicurl"] = 'http://www.youwebsite.com/mp3.mp3';
$msg_arr["hqmusicurl"] = 'http://www.youwebsite.com/mp3_1.mp3';
$YF->SendCustomMsg("openid", "music", $msg_arr);
*/
