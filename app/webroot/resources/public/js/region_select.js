/**
 * new PCAS('detail_location_p', 'detail_location_c', 'detail_location_a', '','','');
 */
SPT = "不限";
SCT = "不限";
SAT = "不限";
...
PCAS.SetC = function (PCA) {
    ...
    for (i = 1; i < PCAC[PI].length; i++) {
        ...
        if (PCA.DefC == PCACV)
            PCA.SelC[PCA.SelP.value && ShowT ? i : i - 1].selected = true
    }
    if (PCA.SelA)
        PCAS.SetA(PCA)
};
PCAS.SetA = function (PCA) {
    ...
    for (i = 1; i < PCAA[PI][CI].length; i++) {
        ...
        if (PCA.DefA == PCAAV)
            PCA.SelA[ShowT ? i : i - 1].selected = true
    }
}
$(function(){
    $(document).on('keyup','#address-name',function(){
        $(this).parents('.form-group').removeClass('has-error').find('label.control-label').text('姓名');
    });
    $(document).on('keyup','#address-tel',function(){
        $(this).parents('.form-group').removeClass('has-error').find('label.control-label').text('手机');
    });
    $(document).on('change',"#address-province,#address-city,#address-area",function(){
        $(this).parents('.form-group').removeClass('has-error').find('label.control-label').text('区域');
    });
    $(document).on('keyup',"address-addr",function(){
        $(this).parents('.form-group').removeClass('has-error').find('label.control-label').text('详细地址');
    });
});
function on_save_addr(callback) {
    var data = {};
    var name = $("#address-name").val();
    if (!name) {
        $("#address-name").focus().parents('.form-group').addClass('has-error').find('label.control-label').text('姓名不能为空');
        callback && callback.error && callback.error();
        return false;
    }
    if (name.length>25) {
        $("#address-name").focus().parents('.form-group').addClass('has-error').find('label.control-label').text('姓名长度不能超过25位');
        callback && callback.error && callback.error();
        return false;
    }
    var tel = $("#address-tel").val();
    if (!tel) {
        $("#address-tel").focus().parents('.form-group').addClass('has-error').find('label.control-label').text('手机不能为空');
        callback && callback.error && callback.error();
        return false;
    }
    var filter  = /^((1[0-9]{1})+\d{9})$/;
    if (!filter.test(tel)) {
        $("#address-tel").focus().parents('.form-group').addClass('has-error').find('label.control-label').text('手机号码格式错误');
        callback && callback.error && callback.error();
        return false;
    }
    var province = $("#address-province").val();
    if (!province) {
        $("#address-province").focus().parents('.form-group').addClass('has-error').find('label.control-label').text('省份不能为空');
        callback && callback.error && callback.error();
        return false;
    }
    var city = $("#address-city").val();
    if (!city) {
        $("#address-city").focus().parents('.form-group').addClass('has-error').find('label.control-label').text('市不能为空');
        callback && callback.error && callback.error();
        return false;
    }
    var area = $("#address-area").val();
    if (!area) {
        $("#address-area").focus().parents('.form-group').addClass('has-error').find('label.control-label').text('地区不能为空');
        callback && callback.error && callback.error();
        return false;
    }
    var addr = $("#address-addr").val();
    if (!addr) {
        $("#address-addr").focus().parents('.form-group').addClass('has-error').find('label.control-label').text('详细地址不能为空');
        callback && callback.error && callback.error();
        return false;
    }
    data.id = $('#address-id').val();
    data.name = name;
    data.tel = tel;
    ...
    $.ajax({
        url: '/account/ajaxSaveAddress',
        data: data,
        type: 'post',
        dataType: 'json',
        success: function (status) {
            callback && callback.success && callback.success(status);
        },
        error: function () {
