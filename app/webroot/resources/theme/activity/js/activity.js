...
function initProductStyle(pid) {
    ...
    pro && $('#detail').html(pro.description) && pro.style && $.each(pro.style, function (index, item) {
        ...
        $.each(item.colors, function () {
            li.append('<div style="background-color:#' + this.name + ';width:' + this.accounting + '%"></div>');
        });
    ...
    product && ($('#detail').html(product.description)) && product.style && $.each(product.style, function (index, style) {
        if (sid == style.id) {
            ...
            for (side in image) {
                ...
                li += (activity && activity.create_image && activity.create_image == '1') ? '<img class="img-responsive" src="' + image[side] + '"/>' : greatSvgPreView(image[side], bgColor, activity.design[side]);
                li += '</li>'
...
function loadProductPhoto(src, bgColor, design) {
    ...
    img.onload = function () {
        ...
        if (activity && activity.create_image && activity.create_image == '1') {
            ...
            photoBox.find('svg').replaceWith(greatSvgPreView(src, bgColor, design));
        }
