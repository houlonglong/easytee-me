...
ui.controllers.SaleGoalController = function($scope){
    ...
    function calcTotalProfitByTotalCount(){
        var machiningCostAll = calcTotalMachiningCost($scope.saleGoal);
        var totalProfits = [];
        for(var i=0; i<$scope.products.length; i++){
            var product = $scope.products[i];
            var unit_price;
            if(product.unit_price){
                ...
                for(var j=0; j<product.product_selected_styles.length; j++){
                    var selectedStyle = product.product_selected_styles[j];
                    unit_price = selectedStyle.unit_price;
                }
            }
            var cost = unit_price + machiningCostAll;
            var price = product.price ? product.price : parseInt(cost)*1.5;
            var profit = price - cost;
            var totalProfit = profit*$scope.saleGoal;
            totalProfits.push(totalProfit);
        }
        totalProfits = quickSort(totalProfits, 0, totalProfits.length-1);
        if(totalProfits.length != 0){
            $scope.preTotalProfit = totalProfits[0] < 0.1 ? 0 : totalProfits[0].toFixed(2);
        }
    }
...
//            alert('预计利润不能小于等于零。');
    ...
    eventManager.on('updateStep2View', function(callback){
        ...
        $scope.products = [];
        $.ajax({
            url: '//' + ezdVars.ApiDomain + '/api?model=design/tool/beta&action=activity_info',
            type: 'post',
            dataType: 'json',
            data: {
                json: 1,
                id: ezdVars.ActivityID
            },
            success: function(data){
