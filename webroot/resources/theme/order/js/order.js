...
function get_express_price_by_province($province){
    ...
    for(i in express_area_price){
        var area = express_area_price[i].area;
        var price = express_area_price[i].price;


        if($province == area){
            price = calculate_express_by_count(price,express_area_price[i].price_xz);
...
function calculate_express_by_count(sPrice,xPrice){
    var count = 0;
    $('.pro_nums').each(function(){
        count += parseInt($(this).val());
    });
    var sWigth = 0.3;
    var totalWight = count*sWigth;
    var price = 0;
    if(totalWight>1){
        a = Math.ceil(totalWight-1);
        price = parseFloat(a*xPrice)+parseFloat(sPrice);
    }else{
        price = sPrice;
    }
